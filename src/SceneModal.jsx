    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-gray-900 border-b border-gray-700 p-6 flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-bold text-purple-400">Выберите сцену</h2>
            <p className="text-gray-400 mt-1">для общения с {character?.name}</p>
          </div>
          <button
            onClick={onClose}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                {availableScenes.map(scene => (
                  ...
                  >
                    ...
                    <div className="p-3">
                      <h3 className="text-lg font-bold mb-1 text-purple-400">{scene.name}</h3>
                      {scene.intro_text && (
                        <p className="text-gray-400 text-sm line-clamp-2">{scene.intro_text}</p>
                      )}
                    </div>
                  </div>
                ))}
